...
pub struct AppState {
    ...
    pub active_jobs: Arc<Mutex<HashMap<String, Arc<Mutex<Child>>>>>,
    pub active_mp4_exports: Arc<Mutex<HashMap<String, Arc<Mutex<Child>>>>>,
...
impl AppState {
    ...
    pub async fn terminate_all_processes(&self) {
        ...
        let active_jobs = {
            ...
            active
                .iter()
                .map(|(job_id, child)| (job_id.clone(), child.clone()))
                .collect::<Vec<_>>()
        };
        for (job_id, child) in active_jobs {
        ...
        for child in active_mp4_exports {
            let _ = child.lock().await.kill().await;
        }
